...
public class HW2Operations
{
	...
	{
		/**
		 * I seperated this method into two parts which will be explained, but I will explain what I wanted to acheive. Ultimately I wanted
		 * to end up with the masks in the form of FFFN, for the nibble mask where N is the nibble, and ABCF, for the number mask, noting that
		 * this is a specific case it doesn't matter which nibble is changed. The reason I did this is because it makes the problem a 
		 * simple and at the end.
		 */
		
		
		/**
		 * To create the nibble mask in the form of FFFN, I first placed set the appropriate space for the nibble to F. Then I negated it so that
		 * when I did an OR with the nibble, which is shifted to the appropriate bit, everything would stay on except for the bits in the
		 * nibble that were off.
		 */
		//Creation of the nibble mask
		int nibbleMask = 0xF << (which * 4);
		nibbleMask = ~nibbleMask;
		nibble = nibble << (which * 4);
		nibbleMask = (nibbleMask | nibble);
		
		/**
		 * The number mask was much simpler because I just needed the nibble spot to be completely on. Once that was shifted into the correct
		 * place, it was a simple OR to correct the other bits.
		 */
		//Creation of the number mask
		int numMask = 0xF << (which * 4);
		numMask = (numMask | num);
	...
	{
		/**
		 * We first have to test if the number is negative usin a simple mask. If the number is negative then it can't be a power of 2.
		 * The key to having a power of 2 is that in binary there can only be one 1 in the entire number. A way to test this is to subtract
		 * one from the binary number, and then AND it with the original. If the answer isn't zero, then there has to be other one's and 
		 * thus it isn't a power of two. We add on another condition for the special case of zero, because zero isn't a power of 2.
		 */
		if((num & 0x80000000) != 0){
			return false;
		}
		return ((num & (num - 0x1)) == 0x0) && (num != 0);
	}
}
